...
class PredictionAggregatorTest < ActiveSupport::TestCase
  ...
  test "for_week should return aggregated prediction counts" do
    ...
    Prediction.create!(user: @user, player: @player, season_type: "weekly", category: "target", gameweek: @gameweek1)
    Prediction.create!(user: @user2, player: @player, season_type: "weekly", category: "target", gameweek: @gameweek1)
    Prediction.create!(user: @user, player: @player2, season_type: "weekly", category: "avoid", gameweek: @gameweek1)

    # Create prediction for gameweek 2 (should not be included)
    Prediction.create!(user: @user2, player: @player2, season_type: "weekly", category: "target", gameweek: @gameweek2)
    ...
    assert_equal 2, player_data["target"]
    assert_equal 0, player_data["avoid"]
    assert_equal 0, player_data["avoid"]

    # Check @player2 predictions
    player2_data = result[@player2.id]
    assert_equal 0, player2_data["target"]
    assert_equal 1, player2_data["avoid"]
    assert_equal 0, player2_data["avoid"]
  end
  ...
  test "for_rest_of_season should return aggregated season prediction counts" do
    ...
    Prediction.create!(user: @user2, player: @player, season_type: "rest_of_season", category: "avoid")
    ...
    # Check @player predictions
    player_data = result[@player.id]
    assert_equal 1, player_data["target"]
    assert_equal 1, player_data["avoid"]
    assert_equal 0, player_data["avoid"]
  ...
  test "for_player should return aggregated predictions for specific player" do
    ...
    Prediction.create!(user: @user, player: @player, season_type: "weekly", category: "target", gameweek: @gameweek1)
    Prediction.create!(user: @user2, player: @player, season_type: "weekly", category: "target", gameweek: @gameweek1)
    Prediction.create!(user: @user, player: @player, season_type: "rest_of_season", category: "avoid")
  ...
  test "for_user should return aggregated predictions for specific user" do
    ...
    Prediction.create!(user: @user2, player: @player, season_type: "weekly", category: "avoid", gameweek: @gameweek1)
  ...
  test "weekly_consensus should return data with Player objects" do
    ...
    assert_equal 2, player_data[:votes]["target"]
    assert_equal 2, player_data[:total_votes]

    player2_data = result[@player2.id]
    assert_equal @player2, player2_data[:player]
    assert_equal 1, player2_data[:votes]["avoid"]
  ...
  test "top_for_week should return top N players for specific category" do
    ...
    Prediction.create!(user: @user, player: @player2, season_type: "weekly", category: "target", gameweek: @gameweek1)

    result = PredictionAggregator.top_for_week(1, "target", 10)
  ...
  test "top_for_week should respect limit parameter" do
    ...
    Prediction.create!(user: @user, player: @player, season_type: "weekly", category: "target", gameweek: @gameweek1)
    Prediction.create!(user: @user, player: @player2, season_type: "weekly", category: "target", gameweek: @gameweek1)
    Prediction.create!(user: @user, player: player3, season_type: "weekly", category: "target", gameweek: @gameweek1)

    # Request only top 2
    result = PredictionAggregator.top_for_week(1, "target", 2)
    assert_equal 2, result.length
  end
  ...
  test "top_rest_of_season should return top N players for specific category" do
    ...
    result = PredictionAggregator.top_rest_of_season("avoid", 10)
  ...
  test "weekly_consensus_by_category should organize data by category" do
    ...
    Prediction.create!(user: @user, player: @player, season_type: "weekly", category: "target", gameweek: @gameweek1)
    Prediction.create!(user: @user2, player: @player2, season_type: "weekly", category: "avoid", gameweek: @gameweek1)
    ...
    # Target category should have @player
    assert_equal 1, result["target"].length
    assert_equal @player, result["target"].first[:player]

    # Avoid category should have @player2
    assert_equal 1, result["avoid"].length
    assert_equal @player2, result["avoid"].first[:player]
  end

  test "rest_of_season_consensus_by_category should organize data by category" do
    # Create rest of season predictions
    Prediction.create!(user: @user, player: @player, season_type: "rest_of_season", category: "target")
    Prediction.create!(user: @user2, player: @player2, season_type: "rest_of_season", category: "avoid")
  ...
  test "should filter out players with zero votes in specific category" do
    ...
    result = PredictionAggregator.top_for_week(1, "avoid", 10)
